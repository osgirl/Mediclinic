...
public class ContactDB
{
    ...
    public static void Delete(int contact_id)
    {
        try
        {
            DBBase.ExecuteNonResult("DELETE FROM Contact WHERE contact_id = " + contact_id.ToString());
        }
    ...
    {
        string sql = "DELETE Contact WHERE entity_id = " + entity_id;
    ...
    public static void Update(int contact_id, int contact_type_id, string free_text, string addr_line1, string addr_line2, int address_channel_id, int suburb_id, int country_id, int site_id, bool is_billing, bool is_non_billing)
    {
        free_text  = free_text.Replace("'", "''");
        addr_line1 = addr_line1.Replace("'", "''");
        addr_line2 = addr_line2.Replace("'", "''");
        string sql = "UPDATE Contact SET free_text = '" + free_text + "',addr_line1 = '" + addr_line1 + "',addr_line2 = '" + addr_line2 + "',contact_type_id = " + contact_type_id + ",address_channel_id = " + (address_channel_id == -1 ? "NULL" : address_channel_id.ToString()) + ",suburb_id = " + (suburb_id == -1 ? "NULL" : suburb_id.ToString()) + ",country_id = " + (country_id == -1 ? "NULL" : country_id.ToString()) + ",site_id = " + (site_id == -1 ? "NULL" : site_id.ToString()) + ",is_billing = " + (is_billing ? "1," : "0,") + "is_non_billing = " + (is_non_billing ? "1," : "0,") + "contact_date_modified = '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' WHERE contact_id = " + contact_id.ToString();
        DBBase.ExecuteNonResult(sql);
    }
    public static void UpdateInactive(int contact_id)
    {
        string sql = "UPDATE Contact SET contact_date_deleted = '" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "' WHERE contact_id = " + contact_id.ToString();
        DBBase.ExecuteNonResult(sql);
    }
    public static void UpdateIsBillingIsNonbilling(int contact_id, bool is_billing, bool is_non_billing, string DB = null)
    {
        string sql = "UPDATE Contact SET is_billing = " + (is_billing ? "1" : "0") + ",is_non_billing = " + (is_non_billing ? "1" : "0") + " WHERE contact_id = " + contact_id.ToString();
    ...
    {
        string sql = "SELECT COUNT(*) FROM Contact WHERE entity_id = " + entity_id;
        return Convert.ToInt32(DBBase.ExecuteSingleResult(sql));
    }

    public static string JoinedSql = @"
                      SELECT 
                             ad.contact_id as ad_contact_id,ad.entity_id as ad_entity_id,ad.free_text as ad_free_text,ad.addr_line1 as ad_addr_line1,ad.addr_line2 as ad_addr_line2,ad.contact_type_id as ad_contact_type_id,ad.address_channel_id as ad_address_channel_id,ad.suburb_id as ad_suburb_id,ad.country_id as ad_country_id,
                             ...
                             ac.address_channel_id as ac_address_channel_id,ac.descr as ac_descr,ac.address_channel_type_id as ac_address_channel_type_id,ac.address_channel_date_added as ac_address_channel_date_added,ac.address_channel_date_modified as ac_address_channel_date_modified, 
                      ...
                      FROM
                             ...
                             LEFT OUTER JOIN AddressChannel     ac   ON ac.address_channel_id      = ad.address_channel_id
                             LEFT OUTER JOIN AddressChannelType act  ON ac.address_channel_type_id = act.address_channel_type_id 
                             LEFT OUTER JOIN Site               site ON site.site_id               = ad.site_id 
                       WHERE 
                             ad.contact_date_deleted IS NULL";


    public static DataTable GetDataTable(int contact_type_group_id, bool orderByShippingBeforeBilling = false)
    {
    ...
    public static Contact[] GetByEntityID(int contact_type_group_id, int entity_id, int contact_type_id = -1, bool orderByShippingBeforeBilling = false, string DB = null)
    {
    ...
    public static Contact[] GetByEntityID(string contact_type_group_ids, int entity_id, string contact_type_ids = null, bool orderByShippingBeforeBilling = false, string DB = null)
    {
        DataTable tbl = GetDataTable_ByEntityID(contact_type_group_ids, entity_id, contact_type_ids, orderByShippingBeforeBilling, DB);
        Contact[] list = new Contact[tbl.Rows.Count];
    ...
    public static Contact[] GetByEntityIDs(int contact_type_group_id, int[] entity_ids, int contact_type_id = -1, int site_id = -1, bool orderByShippingBeforeBilling = false)
    {
    ...
    public static Contact GetFirstByEntityID(int contact_type_group_id, int entity_id, int contact_type_id = -1, bool orderByShippingBeforeBilling = false)
    {
    ...
    public static Contact[] GetByEntityIDs(string contact_type_group_id, int[] entity_ids, string contact_type_id = null, int site_id = -1, bool orderByShippingBeforeBilling = false)
    {
        DataTable tbl = GetDataTable_ByEntityIDs(contact_type_group_id, entity_ids, contact_type_id, site_id, orderByShippingBeforeBilling);
        Contact[] list = new Contact[tbl.Rows.Count];
        for (int i = 0; i < tbl.Rows.Count; i++)
    ...
    {
        Contact[] list = GetByEntityIDs(contact_type_group_id, entity_ids, contact_type_id, site_id, orderByShippingBeforeBilling);
        ...
        for (int i = 0; i < keys.Count; i++)
        {
            ...
            hash[(int)keys[i]] = (Contact[])item.ToArray(typeof(Contact));
        }
    ...
    public static Contact[] GetByAddrLine1(string contact_type_group_ids, string addr_line1, int contact_type_id = -1, bool orderByShippingBeforeBilling = false)
    {
    ...
    public static Hashtable GetHasContact(int[] entityIDs, string contact_type_group_ids, int contact_type_id = -1)
    {
        ...
        string sql = @"SELECT 
                       ...
                       FROM 
                            Contact 
                            INNER JOIN ContactType ON ContactType.contact_type_id = Contact.contact_type_id
                       WHERE 
                            Contact.contact_date_deleted IS NULL
                            AND entity_id in (" + string.Join(",", entityIDs) + @") ";

        if (contact_type_group_ids != null && contact_type_group_ids.Length > 0)
            sql += " AND ContactType.contact_type_group_id IN (" + contact_type_group_ids + ") ";
        if (contact_type_id != -1)
            sql += " AND Contact.contact_type_id = " + contact_type_id;
    ...
    public static Contact GetByID(int contact_id)
    {
    ...
    public static Contact Load(DataRow row, string prefix = "")
    {
        return new Contact(
            ...
            row[prefix + "address_channel_id"] == DBNull.Value ? -1 : Convert.ToInt32(row[prefix + "address_channel_id"]),
