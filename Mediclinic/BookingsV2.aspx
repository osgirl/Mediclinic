<%@ Page Title="Bookings" Language="C#" MasterPageFile="~/SiteV2.master" AutoEventWireup="true" CodeFile="BookingsV2.aspx.cs" Inherits="BookingsV2" %>
...
<asp:Content ID="HeaderContent" runat="server" ContentPlaceHolderID="HeadContent">
    ...
    <script type="text/javascript">
        ...
        function live_search(str) {
            ...
            xmlhttp.onreadystatechange = function () {
                if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                    ...
                    document.getElementById("div_livesearch").style.zIndex = "1999";
                }
            }
        ...
        //Clone the first row and column and place it on the top of the existing table.
        addLoadEvent(function(){
            //Clone the first column (time)
            $("#main_table tr").each(function(index) {
                var newRow = $("<tr valign=\"top\" align=\"left\"></tr>");
                var currTop = $(this).offset().top;
                var nextTop = 0
                if($("#main_table tr").size() != index+1)
                    nextTop = $(this).next().offset().top;
                      
                $(this).find("td:nth-child(1)").each(function() {
                    newRow.append($(this).clone());
                    newRow.css('height',nextTop-currTop + 'px');
                    newRow.css('width',$(this).width());
                    newRow.css('background-color','#ededed');
                    newRow.attr('class','emptyContext');
                    newRow.attr('align','center');
                    minWidth  = $(this).width();
                })
                $("#targetT").append(newRow);
                $("#targetT").css('position','absolute');
                $("#targetT").css('overflow', 'auto');
                $("#targetT").css('min-width', minWidth + "px");
 
            });
            //Clone the header blue left
            $("#header_table tr:first").each(function(index) {
                var newRow = $("<tr valign=\"top\" align=\"left\"></tr>");
                $(this).find("th:nth-child(1)").each(function() {
                    newRow.append($(this).clone());
                    newRow.css('height',($(this).height()) + 'px');
                    newRow.css('width',($(this).width()) );
                    newRow.css('z-index','101');
                    newRow.css('background-color','#ffffff');
                    newRow.attr('class','emptyContext');
                    newRow.attr('align','center');
                })
                $("#targetH").append(newRow);
                $("#targetH").css('position','absolute');
                $("#targetH").css('overflow', 'auto');
                $("#targetH").css('width', $("#targetH").width()+3);
                $("#targetH").css('min-width', $("#targetH").width()+3);
 
            });
        });//]]>
      
 
        //This will make the clones header to stay on top when the user scrolls the page down.
        //and make the clones time column on the left when the user scrolls to the right
        addLoadEvent(function(){
            leftOffsetH=parseInt($("#header_panel").offset().left);
            leftOffsetT=parseInt($("#main_panel").offset().left);
 
            $("#main_panel").scroll(function(e){
                $("#targetT").css("left",  ($(this).scrollLeft()+parseInt($("#main_panel").offset().left) -leftOffsetT) + "px");
            });
            $("#header_panel").scroll(function(e){
                $("#targetH").css("left",  ($(this).scrollLeft()+parseInt($("#header_panel").offset().left) -leftOffsetH) + "px");
            });
 
            $(window).resize(function(e){
                $("#targetT").css("left",  "0px");
                $("#targetH").css("left",  "0px");
                leftOffsetH=parseInt($("#header_panel").offset().left);
                leftOffsetT=parseInt($("#main_panel").offset().left);
        ...
        <div class="clearfix">
            ...
            <div class="main_content" style="padding:10px 5px 20px;">
                ...
                <table class="block_center" id="tbl_booking_sheet" oncontextmenu="return false;">
                    <tr>
                        <td> 
                             <asp:Panel ID="header_panel" runat="server" ScrollBars="Auto" Width="1497" style="position: relative;" onscroll="Javascript:elementOnScroll(this);elementOnScrollLeft(this);">
                                 <%--Create an empty div and table and fill with cloned time column --%>
                                 <table id="targetH" border="1"  runat="server" class="booking-table-grid-header" style="z-index: 101;"></table>
                                 <table border="1" id="header_table" runat="server" class="booking-table-grid-header" style="width:1480px;max-width:1480px;"></table>
                            </asp:Panel>
                              <asp:Panel ID="main_panel" runat="server" ScrollBars="Auto" Height="550" Width="1497" style="min-height:150px; position: relative;" onscroll="JavaScript:elementOnScroll(this);elementOnScrollLeft(this);" >
                                  <%--Create an table and fill with cloned header --%>
                                  <table id="targetT" border="1"  runat="server" class="booking-table-grid-body" style="border-color: #800080"></table>
