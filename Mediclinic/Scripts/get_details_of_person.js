...
function show_patient_detail(d, patient_id) {
    ...
    // use async, otherwise user cant click on link until sync has been done.
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            document.getElementById(d).innerHTML = create_person_table_to_display(items);
            document.getElementById(d).setAttribute("style", "display: block; font-weight: bold; font-size: 13px; font-family: Arial, sans-serif; background-color: #ffffe8; text-align: center; padding: 2px; border: solid 1px; position: absolute;");
        }
    }

    var url_params = new Array();
    url_params[0] = create_url_param("type", "patient");
    url_params[1] = create_url_param("id", patient_id);
    xmlhttp.open("GET", "/AJAX/AjaxGetDetailsOfPerson.aspx?" + create_url_params(url_params), true);
    xmlhttp.send();

    // don't use ajax sync. async doesn't bock user input while waiting to get data
    //
...
function show_register_referrer_detail(d, register_referrer_id) {
    ...
    var url_params = new Array();
    url_params[0] = create_url_param("type", "register_referrer");
    url_params[1] = create_url_param("id", register_referrer_id);
    xmlhttp.open("GET", "/AJAX/AjaxGetDetailsOfPerson.aspx?" + create_url_params(url_params), true);
    xmlhttp.send();
...
// used before for ajax synchronous, but made user input not work, so this isn't used anymore
